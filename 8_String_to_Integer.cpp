...
public:
	int myAtoi(std::string str) {
		...
		int u = 0;	// iterator of p_mum
		const double max_int = pow(2, 31) - 1;
		const long min_int = -pow(2, 31);
		char * p_num = new char[len]();
		bool minus_flag = false;
		bool continual_numbers_flag = false;
		double real_num = 0;
		...
		for (i = 0; i < u; i++) {
			real_num = real_num * 10 + (p_num[i] - 48);
		}
		if (minus_flag) {
			real_num *= -1;
		}
